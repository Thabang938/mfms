...
export default function TiresPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [tRes, vRes] = await Promise.all([
        ...
        supabaseClient.from('vehicles').select('id, registration_number, make, model, odometer'),
      ]);
      ...
      console.error('Error fetching tyres:', err.message);
      setTires([]);
      setVehicles([]);
    } finally {
      setLoading(false);
  ...
  const filtered = useMemo(() => {
    ...
    return tires.filter(t => {
      ...
      return `${vehicleLabel} ${t.position || ''} ${t.brand || ''} ${t.serial_number || ''} ${t.condition || ''}`.toLowerCase().includes(q);
    });
  ...
  const exportCSV = () => {
    ...
    const rows = paged.map(t => {
      ...
      // mileage uses vehicle odometer if available
      const mileage = v.odometer != null ? `${Number(v.odometer).toLocaleString()} km` : (t.mileage != null ? `${t.mileage.toLocaleString()} km` : '');
      return {
      ...
      <main className="flex-1 p-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={exportCSV} className="flex items-center bg-white border border-green-300 text-green-700 px-4 py-2 rounded hover:bg-green-100 transition"><FaDownload className="mr-2" /> Export</button>
            <button onClick={() => setOpenAdd(true)} className="flex items-center bg-green-700 text-white px-4 py-2 rounded hover:bg-green-800 transition"><FaPlus className="mr-2" /> Record Tyre Change</button>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          <div className="relative">
            ...
            <input aria-label="Search tyres" className="pl-10 pr-4 py-2 border rounded-md w-64 focus:ring-2 focus:ring-green-200 bg-white transition"
              placeholder="Search vehicle, position, brand, condition..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
          </div>
        </div>
            ...
            <AnimatePresence>
              <tbody>
                ...
                {!loading && paged.map((tire, i) => {
                  ...
                  const mileage = vehicle.odometer != null ? `${Number(vehicle.odometer).toLocaleString()} km` : (tire.mileage != null ? `${Number(tire.mileage).toLocaleString()} km` : '—');
                  return (
                    <motion.tr key={tire.id || i} className="border-t hover:bg-green-50 cursor-pointer" onClick={() => router.push(`/Dashboard/tyres/${tire.id}`)} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                      ...
                      <td className="px-4 py-3">{mileage}</td>
                      <td className="px-4 py-3">{tire.pressure != null ? `${Number(tire.pressure).toFixed(2)} bar` : '—'}</td>
                      <td className="px-4 py-3"><span className={`px-2 py-1 rounded text-xs font-semibold ${tire.condition === 'Good' ? 'bg-green-200 text-green-800' : tire.condition === 'Fair' ? 'bg-yellow-200 text-yellow-800' : 'bg-red-200 text-red-800'}`}>{tire.condition}</span></td>
                    </motion.tr>
                  );
                })}
              </tbody>
...
// StatCard used in tyres page
function StatCard({ label, value, color }) {
  return (
    <motion.div className="bg-white p-4 rounded-lg border border-green-100 shadow-sm text-center hover:shadow-md transition" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}>
